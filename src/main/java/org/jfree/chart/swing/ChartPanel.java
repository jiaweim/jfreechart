        ...
        MouseMotionListener, OverlayChangeListener, Printable, Serializable
{
    ...
    /**
     * Default setting for buffer usage.  The default has been changed to {@code true} from version 1.0.13 onwards,
    ...
    /**
     * The standard mouse button modifiers for alternative drag operations. There are two kinds of mouse drag
     * operations: pan and zoom. To distinguish between them, one needs to require modifier keys to be held down during
     * the dragging.  However, some modifiers may not be usable on all platforms.  For example, on Mac OS X it is
     * impossible to perform Ctrl-drags or right-drags, see
     * <a href="http://developer.apple.com/qa/qa2004/qa1362.html">http://developer.apple.com/qa/qa2004/qa1362.html</a>.
     * This function returns a non-zero modifier usable for any platform: Alt for Mac OS X, Ctrl for other platforms.
     * It is recommended to use these modifiers for one operation, and zero modifiers for the other.
     *
    ...
    /**
     * The default mask for mouse events to trigger panning. Since 2.0.0, this mask uses extended modifiers, as returned
     * by {@link InputEvent#getModifiersEx()}. Only used if no button-specific modifiers were set in
     * {@link #panButtonMasks}.
     */
    ...
    /**
     * Constructs a panel containing a chart.  The {@code useBuffer} flag controls whether or not an offscreen
     * {@code BufferedImage} is maintained for the chart.  If the buffer is used, more memory is consumed, but panel
     * repaints will be a lot quicker in cases where the chart itself hasn't changed (for example, when another frame is
     * moved to reveal the panel).  WARNING: If you set the {@code useBuffer} flag to false, note that the mouse zooming
     * rectangle will (in that case) be drawn using XOR, and there is a SEVERE performance problem with that on JRE6 on
     * Windows.
     *
     * @param chart     the chart.
     ...
     * @param useBuffer         a flag that indicates whether to use the off-screen buffer to improve performance (at
     *                          the expense of memory).
     * @param properties        a flag indicating whether or not the chart property editor should be available via the
     *                          popup menu.
     * @param save              a flag indicating whether or not save options should be available via the popup menu.
     * @param print             a flag indicating whether or not the print option should be available via the popup
     *                          menu.
     * @param zoom              a flag indicating whether or not zoom options should be added to the popup menu.
     * @param tooltips          a flag indicating whether or not tooltips should be enabled for the chart.
     */
     ...
     * @param fillRectangle {@code true} if zoom rectangle is filled, false if rectangle is shown as outline only.
     */
    ...
    /**
     * Sets default modifier keys for pan operations for all mouse buttons. Modifiers for a specific button can be set
     * with {@link #setPanModifiersEx(int, int)}.  If there are none set for a certain button, it will use the modifiers
     * passed to this function, defaulting to {@link #getDefaultDragModifiersEx()} if this function was never called.
     * <p>
     * Only {@link InputEvent#SHIFT_DOWN_MASK}, {@link InputEvent#CTRL_DOWN_MASK}, {@link InputEvent#META_DOWN_MASK} and
     * {@link InputEvent#ALT_DOWN_MASK} are checked.  To avoid platform-specific problems, it is recommended to use
     * {@link #getDefaultDragModifiersEx()} for one operation, and zero modifiers for the other.
     * <p>
     * If the same modifiers are set for both zooming and panning, panning will be performed.
     *
    ...
    /**
     * Sets default modifier keys for zoom operations for all mouse buttons. Modifiers for a specific button can be set
     * with {@link #setZoomModifiersEx(int, int)}.  If there are none set for a certain button, it will use the
     * modifiers passed to this function, defaulting to zero (no modifiers) if this function was never called.
     * <p>
     * Only {@link InputEvent#SHIFT_DOWN_MASK}, {@link InputEvent#CTRL_DOWN_MASK}, {@link InputEvent#META_DOWN_MASK} and
     * {@link InputEvent#ALT_DOWN_MASK} are checked.  To avoid platform-specific problems, it is recommended to use
     * {@link #getDefaultDragModifiersEx()} for one operation, and zero modifiers for the other.
    ...
    /**
     * Sets modifier keys for panning with a specific mouse button. If there are none set for a certain button with this
     * function, default modifiers set with {@link #setDefaultPanModifiersEx(int)} will be used, defaulting to
     * {@link #getDefaultDragModifiersEx()} if none were set either.<p> Only {@link InputEvent#SHIFT_DOWN_MASK},
     * {@link InputEvent#CTRL_DOWN_MASK}, {@link InputEvent#META_DOWN_MASK} and {@link InputEvent#ALT_DOWN_MASK} are
     * checked.  To avoid platform-specific problems, it is recommended to use {@link #getDefaultDragModifiersEx()} for
    ...
    /**
     * Sets modifier keys for zooming with a specific mouse button. If there are none set for a certain button with this
     * function, default modifiers set with {@link #setDefaultZoomModifiersEx(int)} will be used, defaulting to zero (no
     * modifiers) if none were set either.
     * <p>
     * Only {@link InputEvent#SHIFT_DOWN_MASK}, {@link InputEvent#CTRL_DOWN_MASK}, {@link InputEvent#META_DOWN_MASK} and
     * {@link InputEvent#ALT_DOWN_MASK} are checked.  To avoid platform-specific problems, it is recommended to use
     * {@link #getDefaultDragModifiersEx()} for one operation, and zero modifiers for the other.
    ...
    /**
     * Sets the flag that controls whether or not zooming is enabled for the domain axis.  A check is made to ensure
     * that the current plot supports zooming for the domain values.
     *
     ...
     *
     * @param flag {@code true} instructs to fill the rectangle on zoom, otherwise it will be outlined.
     */
    ...
    /**
     * Sets the default directory for the "save as" option.  If you set this to {@code null}, the user's default
    ...
    /**
     * Returns a point based on (x, y) but constrained to be within the bounds of the given rectangle.  This method
     * could be moved to JCommon.
     *
     ...
     * Opens a file chooser and gives the user an opportunity to save the chart in PNG format.
     *
     * @param w the width for the saved image (if less than or equal to zero, the panel width will be used);
     * @param h the height for the PNG image (if less than or equal to zero, the panel height will be used);
     * @throws IOException if there is an I/O error.
     */
     ...
     * @param width  the width.
     * @param height the height.
     * @return A string containing an SVG element for the current chart, or {@code null} if there is a problem with the
     * method invocation by reflection.
     */
    ...
    /**
     * Creates an {@code SVGGraphics2D} instance (from JFreeSVG) using reflection. If JFreeSVG is not on the classpath,
    ...
    /**
     * Creates a popup menu for the panel.  This method includes code that auto-detects JFreeSVG and OrsonPDF (via
     * reflection) and, if they are present (and the {@code save} argument is {@code true}, adds a menu item for each.
     *
