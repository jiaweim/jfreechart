...
public class NumberAxisProps {
    ...
    /**
     * Return the {@link XYChart} that the axis belongs to.
     * Throw {@link IllegalStateException} if the original chart is not an XYChart.
     *
     * @return {@link XYChart}.
     */
    public XYChart done() {
        if (chart_ instanceof XYChart xyChart) {
            return xyChart;
        }
        throw new IllegalStateException("");
    }

    public CategoryXYChart categoryChart() {
        if (chart_ instanceof CategoryXYChart categoryChart) {
